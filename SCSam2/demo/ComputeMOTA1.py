...
if __name__ == "__main__":
    ...
    mask1 = np.zeros((tmp.shape[0], tmp.shape[1]))
    mask2 = np.zeros((tmp.shape[0], tmp.shape[1]))
    mask3 = np.zeros((tmp.shape[0], tmp.shape[1]))
    mask4 = np.zeros((tmp.shape[0], tmp.shape[1]))
    mask5 = np.zeros((tmp.shape[0], tmp.shape[1]))
    ...
    for i in range(num_objs):
        ...
        for cam in range(len(cam_list)):        
            for frame in range(num_frame):            
                ...
                if t1 is not None:
                    ...
                    mask3 = cv2.imread(filename3, cv2.IMREAD_GRAYSCALE)
                    mask4 = cv2.imread(filename4, cv2.IMREAD_GRAYSCALE)
                    mask5 = cv2.imread(filename5, cv2.IMREAD_GRAYSCALE)
        ...
        if total_gt1 > 0:
            motas1.append(1.0 - (total_fp1 + total_fn1 + total_idsw1) / float(total_gt1))
            motas2.append(1.0 - (total_fp2 + total_fn2 + total_idsw2) / float(total_gt2))
            motas3.append(1.0 - (total_fp3 + total_fn3 + total_idsw3) / float(total_gt3))
            motas4.append(1.0 - (total_fp4 + total_fn4 + total_idsw4) / float(total_gt4))
        
        #print(cam, 1.0 - (total_fp1 + total_fn1 + total_idsw1) / float(total_gt1))

    
    print(np.mean(motas1))
    print(np.mean(motas2))
    print(np.mean(motas3))
    print(np.mean(motas4))
