...
class SCSam3VideoPredictorNewMem:
    ...
    def __init__(
        ...
        from build_scsam3 import build_scsam3_video_model_newmem
    ...
    def handle_stream_request(self, request):
        ...
        if request_type == "propagate_in_video":
            yield from self.propagate_in_video(
                ...
                spatial_idx=request["spatial_idx"],
    ...
    def propagate_in_video(
        ...
        logger.debug(
            f"propagate in video in session {session_ids[spatial_idx]}: "
            f"{propagation_direction=}, {start_frame_idx=}, {max_frame_num_to_track=}"
        )
        try:
            inference_states = []
            for id in session_ids:
                inference_states.append(self._get_session(id)["state"])
            ...
            logger.debug(
                f"propagation ended in session {session_ids[spatial_idx]}; {self._get_session_stats()}"
            )
...
class SCSam3VideoPredictorMultiGPUNewMem(SCSam3VideoPredictorNewMem):
