...
문헌은 2023년 이후. 후보 생성기(G0 현행 / G1 순서 변형 / G-det 예시 검출–연결(실현성 스파이크 선행) / 🔒 G-geo 비보정
기하 투영 / 🔒 G-corr 대응 프롬프트) → 위치 기준 검증·선택(SeedVote, τ=0.5 고정, 교체는 주석 카메라 2대) → 본류
`--seeds-from` 훅. 실험 S1-E0~E8(층: 탐색/내부 확인/확정)과 판정 초안, 배관, 일정은 그 문서에 있고 노션 개발 ›
SCSegmentation › "1단계 구현 계획"과 동기화합니다. v2(09-10)는 세 관점의 반박 검토를 반영했습니다. **사용자 확인
대기**: "SAM 3만"이 기하·특징 모델(π³/VGGT, DINOv2)의 추론 사용까지 허용하는지 — 확인 전까지 G-geo·G-corr·E8은 🔒.
