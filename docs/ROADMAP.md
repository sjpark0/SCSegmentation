...
## 채택된 구성 — 2026-09-10 갱신: `SegMaskSam3XW1CGPS4MFsCs10kG2`

**`--xview-window 1 --xview-mode C --xview-gate --xview-ptr --xview-tpos-shift 4 --ref-cam muvod --frame0-seed --crop-small 10000 --crop-gate v2`**
→ MUVOD basic **91.0** / complete **86.4** ([stage2-R2.md](stage2-R2.md)). 분해(basic, XMem 79.4 기준): 기초 모델·2단계 구조 +10.2 →
이웃 읽기 +0.5 → 프레임 0 시드 +0.03 → **작은 객체 크롭 추적 +0.98** = +11.7. 아래는 그 이전 이력입니다.
...
| 2026-09-10 | **S1-E2-0 — G-det 실현성 스파이크** ([stage1-E2-0.md](stage1-E2-0.md); 등록 2918177, GPU 10분). 동결 SAM 3의 `visual_prompt_embed` 슬롯은 있으나 배선이 없음 → 인스턴스 래핑으로 c_ini 박스의 예시 토큰을 다른 뷰의 검출기에 주입. **교차 주입은 됨**: 등록 20쌍에서 정답 객체 IoU≥0.5 후보 17/20(대조 0/20, 1위 13), 핀홀 20쌍 16/20. 그러나 등록 기준 ① 같은 이미지 재현 IoU>0.8이 15/20=0.75로 미달(실패 5쌍 중 3쌍은 화면 11~34%의 배경 띠·바닥) → **규칙상 폐기**. 예측(②는 0.3~0.6) 크게 빗나감 — 예시가 뷰를 넘어 옮겨진다. 재등록(① 모집단을 화면 10% 미만 객체로) 여부는 사용자 결정 |
| 2026-09-10 | **S2-R2 — 크롭 추적 본류 구현·실행, 새 헤드라인 basic 91.0** ([stage2-R2.md](stage2-R2.md); 등록 2cdba1e → 구현 ee75662(테스트 436) → 17장면 약 80분). `--crop-small 10000 --crop-gate v2`를 `runMVSeg.py`에 구현(쓰기 루프 뒤, 채점 카메라의 작은 객체를 같은 예측기의 크롭 세션으로 추적해 프레임 1~20만 덮어씀; 이웃 기억과는 분리). 새 헤드라인 `SegMaskSam3XW1CGPS4MFsCs10kG2`: basic 90.034 → **91.015**(+0.981, CI [+0.446, +1.613], **16↑/0↓**), complete 85.5 → **86.4**, XMem 대비 +11.7. 구현 검증: 대상/되돌림 349/71이 R1 조립과 17/17 동일, 비대상 PNG 16,051/16,051·채택 대상 PNG 5,254/5,254 바이트 동일 → 조립 추정과 정확히 일치. 하네스 함정: 모드 C 큰 장면은 600초를 넘겨 배경 태스크가 죽지만 컨테이너는 완주 — 분리 드라이버(setsid nohup)로 실행. **1단계로 복귀** |
| 2026-09-10 | **S2-R1 — 작은 객체 크롭 추적(2단계 유효 해상도)** ([stage2-R1.md](stage2-R1.md); 등록 7fe88dd → 게이트 v1 9fa8d88 → v2 1aeb99e, GPU 약 30분). 시드 박스 4배 창(최소 256 px)으로 잘라 SAM 3 시간 추적 후 되붙임. 기준 카메라(정답 시드) <2k px 36쌍의 프레임 1~20 J **0.637 → 0.842**. 17장면 basic(대조군 `XW0MFs` 89.549): Cr2k +0.96, Cr10k +1.28이나 창 안의 다른 물체로 뛰는 쌍이 방어선을 깸(CBABasketball −1.23, Painter −1.88) → 정답 없는 게이트: v1(프레임 1 마스크·시드 IoU<0.05 되돌림) Cr10k **+1.38**(CI [+0.66, +2.20]) MartialArts −0.36으로 0.06 차 미달; v2(+줌<1.2·창 이탈≥10) **+1.05**(CI [+0.51, +1.67], 13↑/1↓, 최악 −0.10) → **관문 통과(탐색)**. 지금까지 잰 어떤 손잡이보다 큼(이웃 읽기 전체 +0.5). E7 구성 후보(`--frame0-seed` + `--crop-small 10000` + 게이트 v2), 모드 C 상호작용은 E7에서. 결함 정정 1건(게이트가 시드를 폴더 프레임 0에서 읽던 것 → 프롬프트 원천). 사용자 결정대로 **1단계로 복귀** |
| 2026-09-10 | **S1-E0″ — 프레임 0을 원시 시드로** (사용자 제안; 등록 a79666b → 오프라인 치환 채점, GPU 0). 프레임 1~20은 체인상 불변이므로 기존 출력의 프레임 0 PNG를 시드로 바꿔 채점: XW0M 89.519→89.549, XW1CGPS4M 90.034→**90.064**, **+0.030점**, CI [+0.022, +0.039], 17/17 상승 → `--frame0-seed`(접미 `Fs`, 커밋 a8fc158, 테스트 385)를 E7 구성에 포함. 진단: 격자 손실은 매 프레임에 있음(기준 카메라 <2k px 프레임 0 0.822 → 1~20 0.632), 봉투 +0.41(<2k)~+0.66점(<10k) → 크롭 추적(S2-R1) 후보 |
| 2026-09-10 | **1단계 1주차: 배관 + S1-E0·F0·E0′·E1** (등록 0b64e20 → 배관 844da27, 테스트 225→362 → GPU 51회 약 25분). 배관: 전 뷰 시드 writer·매니페스트, score_seeds 664형/796형·크기·리그·거리·짝지은 CI, 본류 `--seeds-from`(접미 `Sd<tag>`, 거부 규칙, 폴백 G0), 왕복 재현기, 회귀·동결 스크립트. **E0**: 원시 시드 J **0.8426**(프레임 0 0.8361, 재예측 −0.0065, 새 실패 4), 회귀 R² 0.922. **F0 = 41**(빈 3·자리 틀림 38·핀홀∧≥2k 16), N0 = 623 동결. **E0′**: 천장 0.951은 **288 격자 왕복 인공물** — 마스크 경로에 SAM 디코더 없음, CPU 재현 0.9511 = 측정(367/398 픽셀 동일); 1회 왕복 0.970, 2회 0.951, <2k px 0.822 → 새 열린 항목. **E1**: `reverse` 후보 유지(F0 5 회복, N0 CI [−0.0019, +0.0018]), `ref_outward` 탈락(N0 CI [−0.0229, −0.0005], Blocks −0.094). 예측 8개 중 5 적중 ([stage1-E0.md](stage1-E0.md)) |
