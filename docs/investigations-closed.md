...
| **OneStageNew(무패치 원본) 신규 재실행 vs MVOpt 저장본** (2026-09-04) | PNG 12,428장 | **0** |

★ 이 대조는 **수정 반영 이전**에 수행한 것입니다. 지금 같은 명령을 돌리면
패치 코드끼리 비교하게 되어 아무것도 증명하지 못합니다.
...
## 11-A. 원본 OneStageNew는 Welder를 90 GiB 안에서 완주하지 못했습니다 (2026-09-04 발견, 같은 날 해소)

> **재현 불가 안내.** 아래는 **무패치 원본 코드**의 동작입니다.
> 지금 `--algo OneStageNew`를 돌리면 Welder가 **완주합니다** — 그 폴더가 이제 MVOpt 코드이기 때문입니다.
> 원본으로 재현하려면 `git checkout baseline-onestagenew -- SCSam3/demoSCSam3OneStageNew`가 필요합니다.
...
**실증 (2026-09-04, 수정 반영 후 같은 조건).**

| | 반영 전 | 반영 후 |
|---|---|---|
| Welder | exit 137 (OOM), 941장 중 677장에서 중단 | **ok, 941장 완주** |
| MVOpt와 대조 | — | **941 대 941 완전 일치** |
| `spatial model retired` 로그 | 없음 | 출력됨 |
