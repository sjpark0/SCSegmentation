...
### 논문이 명시하지 않아 우리가 정한 두 가지

1. **카메라에 아예 없는 객체.** basic 집합은 c_ini의 첫 프레임으로 정해지는데, 그 객체가 다른 카메라에는
   한 프레임도 안 나올 수 있습니다. 우리는 **카메라마다 그 카메라의 정답에 실제로 나오는 객체만** 셉니다.
   대안(없는 객체도 세어 빈 예측에 1.0을 주는 것)은 DAVIS 규약상 만점이 되어 점수를 부풀립니다.
   논문 표만으로는 둘을 가릴 수 없어 부풀리지 않는 쪽을 택했습니다.
2. **프레임 범위.** DAVIS 원본 코드는 객체마다 첫·마지막 프레임을 버립니다. 논문은 "21프레임"이라고만
   쓰므로 **21장 전부**를 씁니다(`--frames all`). `--frames inner`로 뺀 값도 볼 수 있지만 대외 수치는
   전부 쪽입니다.
...
`muvod.json`(배포 색인)의 주석 오프셋·뷰 수·인스턴스 수를 우리 설정과 맞춰 봤습니다.
**17개 중 16개가 정확히 일치**합니다. 프레임은 17개 전부 카메라당 21장이고 프레임 번호 구간도 설정과 같습니다.

한 곳만 다릅니다 — **Welder**의 뷰 수를 색인은 41, 우리 설정은 46으로 적고 있습니다. 디스크에는
`camera_0001`~`camera_0046` 46개가 실제로 있어 설정이 디스크와는 맞습니다. 그리고 이 장면의 c_ini는
`camera_0001`(뷰 인덱스 0)이고 채점 카메라가 인덱스 0·2·3이라 closure가 세션 0~3만 돌립니다.
즉 **인덱스 4 이상의 카메라는 결과에 전혀 관여하지 않으므로 이 차이는 출력에 영향이 없습니다.**
