...
## 0. SMB 자격증명 교체 ★ 미해결 (2026-09-09)

`mount_250620.sh`에 내부 호스트 주소와 SMB 계정·비밀번호가 평문으로 들어 있습니다.
**커밋 `68b92ca`(2025-11-27)부터 공개 원격 `main`에 올라가 있었습니다.**

2026-09-09에 한 것 — `git rm --cached`로 추적에서 빼고 `.gitignore`에 `mount_*.sh`를 넣어
푸시했습니다(커밋 `a70bcb3`). 원격 최신 트리에는 없습니다. 로컬 파일은 그대로 둡니다.

**하지 않은 것, 그래서 남은 위험** — 이력에는 그대로 있습니다. `68b92ca` 이후 어느 커밋이든
열면 값이 읽힙니다. **자격증명 교체가 필요하며 이것은 사용자 작업입니다.**

이력에서까지 지우려면 커밋 재작성 + 강제 푸시가 필요합니다(2026-09-07 HuggingFace 토큰
때와 같은 방법, [operations.md](operations.md) 함정 8). 공개 저장소에 대한 강제 푸시이고
이미 복제된 사본에는 남으므로 **교체가 먼저이고, 재작성은 그다음 선택 사항**입니다.
